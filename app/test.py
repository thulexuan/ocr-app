...
from vietocr_tool.vietocr.tool.predictor import Predictor
from vietocr_tool.vietocr.tool.config import Cfg
...
def text_detection_by_craft(image_path):
    ...
    print('done work of result folder')
    ...
    for i in range(1, len(bboxes)):
        max_of_previous_line = current_subarray[-1][-1][1]
        # y4_min của dòng sau >= y4_max dòng trước + chiều cao min của 1 từ trong văn bản
        if bboxes[i][-1][1] - max_of_previous_line < min_space_between_lines:
...
def process_images_in_folder(folder_path):
    ...
    # config = Cfg.load_config_from_file('./vietocr_tool/config/vgg-transformer.yml')
    config['device'] = 'cpu' # device chạy 'cuda:0', 'cuda:1', 'cpu'
    
    # print('print config weight here:')
...
def main():
    # args = parse_arguments()
    text_detection_by_craft('./test/test.jpg')
    recognized_text = process_images_in_folder('./output_images')
